...
const handleProcessOutput = (childProcess, options) => {
	return new Promise((resolve, reject) => {
		const { prefix, resolvePatterns = [], rejectPatterns = [], closeMessage, pipeCallback } = options;
		...
		childProcess.stdout.on("data", (chunk) => {
			...
			for (const pattern of rejectPatterns) {
				...
				if (match) {
					reject(
						new Error(`${prefix}: reject pattern ${pattern} failed with match: ${match}`, {
							pattern,
							match
						})
					);
...
const bindService = async (remoteServiceName, servicePath, cdsrcExists, options = {}) => {
	const outputOptions = {
		prefix: `[bindService][${servicePath}]`,
		resolvePatterns: [new RegExp(`Saving bindings to .cdsrc-private.json in profile ${options.profile}`)]
	};
	const log = createLogger(outputOptions.prefix);
	outputOptions.pipeCallback = options.verboseOutput ? log : undefined;
	const cdsrcContent = cdsrcExists ? await fs.readFile(`${servicePath}/.cdsrc-private.json`, "utf8") : "";
	if (cdsrcExists && cdsrcContent.includes(remoteServiceName)) {
		options.verboseOutput && log(`${servicePath}/.cdsrc-private.json already exists. Skipping binding of ${remoteServiceName}.`);
		return;
	}
...
const bindServices = async (service, servicePath, options = {}) => {
	...
	try {
		...
		for (const boundService of options.boundServices) {
			await bindService(boundService, servicePath, cdsrcExists, { verboseOutput: verboseOutput, profile: options.profile });
		}
	} catch (error) {
...
const loadService = async (servicePath, options) => {
	const { port, debugPort = Number(port) + 1000, winDelay = 500, debug, resolvePatterns = [], rejectPatterns = [], verboseOutput = false } = options;
	const outputOptions = {
	...
	debug && log(`Service ${servicePath} debugging enabled on 'http://localhost:${debugPort}'`);
...
const loadServer = async (options = {}) => {
	...
	const approuter = spawn("cds", ["bind", "--exec", "--", "npm", "start", "--prefix", "./router", "2>&1"], {
		shell: true,
	...
	log(`For opening local launchpad: 'http://localhost:8008/$launchpad?sap-ui-xx-viewCache=false'`);
	return approuter;
};
...
const createShutdownHandler = (spawnedProcesses, options = {}) => {
	...
	return async () => {
		...
		spawnedProcesses.forEach((proc) => {
			if (proc) {
				verboseOutput && console.log(`Attempting to terminate process ${proc.pid}, spawnargs: ${proc.spawnargs}`);
				try {
					proc.kill("SIGTERM");
					verboseOutput && console.log(`Successfully sent SIGTERM to process ${proc.pid}`);
				} catch (err) {
					console.error(`Error terminating process ${proc.pid}:`, err);
					try {
						proc.kill("SIGKILL"); // Forceful termination if SIGTERM fails
						verboseOutput && console.log(`Forcefully killed process ${proc.pid}`);
					} catch (sigkillerr) {
						console.error(`Error forcefully terminating process ${proc.pid}:`, sigkillerr);
					}
				}
			}
		});
...
const main = async (options) => {
	...
	const userArgs = {
		...
		withBindingsUpdate: myArgs.includes("--with-bindings-update") || options.withBindingsUpdate,
	...
	try {
		...
		const serviceTasks = options.services.map((service) => {
			return { name: options.basePath + service.path, options: { ...defaultServiceOptions, port: service.port, profile: options.profile } };
		});
		...
		await Promise.all(setupAndBuildTasks.map((task) => task({ verboseOutput: userArgs.verboseOutput, ...options })));
		...
		await shutdown();
		!userArgs.verboseOutput && console.error("Use --verbose flag to get detailed output");
